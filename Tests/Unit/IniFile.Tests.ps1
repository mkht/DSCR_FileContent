...
"@
    ...
    Describe 'IniFile/Get-TargetResource' {
        BeforeAll {
            Mock -ModuleName IniFile Get-IniFile {
                return $MockIniObject1
            }

            Mock -ModuleName IniFile Test-Path { $true } -ParameterFilter { $Path -eq $ExistMock }
            Mock -ModuleName IniFile Test-Path { $false } -ParameterFilter { $Path -eq $NonExistMock }
        }
    ...
    Describe 'IniFile/Test-TargetResource' {
        ...
        Context 'Ensure = Present' {
            BeforeAll {
                Mock -ModuleName IniFile Test-Path { $true }
            }
